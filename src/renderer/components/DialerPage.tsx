        ...
        <main className="flex-1 flex flex-col min-h-0 overflow-visible">
          ...
          <div className="flex-1 min-h-0 mt-2.5 flex flex-col overflow-visible">
            <div className="flex-1 min-h-0 flex items-center justify-center overflow-visible">
              <div className="grid grid-cols-3 gap-2 aspect-[3/5] h-[80%] max-h-full w-auto max-w-full [grid-template-rows:repeat(5,minmax(0,1fr))]">
                {KEYPAD_KEYS.map((row) =>
                  row.map((key) => (
                    <motion.button
                      key={key}
                      ...
                      transition={{ duration: 0.1 }}
                      className="aspect-square w-full max-w-full h-full max-h-full rounded-full text-white text-lg font-medium flex items-center justify-center min-w-0 min-h-0"
                      style={{
                        WebkitAppRegion: 'no-drag' as React.CSSProperties['WebkitAppRegion'],
